...
uniform sampler2DRect src, srcBlur, thomasBlur;
...
void main() {
	...
	if(srcColorBlur.r > 0.) { // don't copy anything black
		...
		vec4 dstColorBlur = texture2DRect(thomasBlur, pos);// crear un vec4f llamado dstColorBlur en funcion de dstBlur y pos
